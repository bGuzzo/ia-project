...
private:
    void do_work()
    {
        ...
        {
            progress_ += 0.2;
            send_feedback(progress_, "Robot " + arguments[0] + " is moving with carrier " + arguments[1] + " from zone " + arguments[2] + " to zone " + arguments[3]) + " [" << std::min(100.0, progress_ * 100.0) << "%]  \n" << std::flush;
        }
        else
        {
            finish(true, 1.0, "Robot " + arguments[0] + " is moving with carrier " + arguments[1] + " from zone " + arguments[2] + " to zone " + arguments[3]) + " [" << std::min(100.0, progress_ * 100.0) << "%]  \n" << std::flush;
            progress_ = 0.0;
            std::cout << std::endl;
        }
    }
